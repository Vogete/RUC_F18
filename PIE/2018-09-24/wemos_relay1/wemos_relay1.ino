...
void setup() {
  ...
  addLightPattern(relayHigh, "Relay High");
  addLightPattern(relayLow, "Relay Low");
...
void off()
{
  ...
  // annoying green blinking indicator
...
void freePattern()
{
  // if the button is pressed (button connected to D6 and GND), change to Red pattern
  boolean btnPressed = !digitalRead(D6);
  if (btnPressed) {
    lightPatternSelected = 1;
  }
  ...
  for (int i = 0; i < NUM_LEDS; i++)
  {
    leds[i] = CHSV(rainbowColor, 255, 255);
  }
...
void disco(){
  for (int i = 0; i < NUM_LEDS; i++) {
    leds[i] = CHSV(random(0, 255), random(0, 255), 255);
  }
}


void relayHigh(){
  digitalWrite(RELAY_DATA_PIN, HIGH);
}
